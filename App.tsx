...
const App: React.FC = () => {
  ...
  const [users, setUsers] = useState<Profile[]>([
    { id: 'u1', email: 'alex@varsity.edu', role: UserRole.VENDOR, fullName: 'Alex Johnson', university: 'University of Ghana (Legon)', status: 'Active' },
    { id: 'u2', email: 'sarah@campus.edu', role: UserRole.VENDOR, fullName: 'Sarah Chen', university: 'Kwame Nkrumah University (KNUST)', status: 'Active' },
  ...
  const handleOnAuthSuccess = (user: Profile) => {
    ...
    if (user.email === 'admin@unimall.edu.gh') {
      ...
      logAudit('Admin Login', `Admin user logged in from ${user.university}`, 'Medium');
    } else {
  ...
  const handleApproveVendor = (appId: string) => {
    ...
    notify(`Node ${app.storeName} is now live.`, "success");
    logAudit('Vendor Approved', `Application ${appId} approved for user ${app.fullName}`, 'High');
  };
        ...
        <div className="flex items-center space-x-6 md:space-x-10">
          <nav className="hidden md:flex space-x-8">
            ...
            {currentUser?.role === UserRole.ADMIN && (
              <button onClick={() => setView(View.ADMIN_PANEL)} className="text-[10px] font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-700 font-bold border-b-2 border-indigo-500 pb-1">Console</button>
            )}
            ...
            <div className="flex flex-col h-full">
               ...
               {cart.length > 0 && (
                 <div className="p-10 bg-zinc-950 text-white rounded-t-[48px]">
                    ...
                    <button className="w-full bg-white text-zinc-950 py-6 rounded-3xl font-black uppercase tracking-widest text-xs hover:bg-amber-400 hover:scale-[1.02] transition-all shadow-2xl">Execute protocol</button>
                 </div>
               )}
            </div>
         </div>
