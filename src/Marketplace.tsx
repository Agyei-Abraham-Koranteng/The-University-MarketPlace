...
const Badge = ({ children, color = 'zinc' }: { children: React.ReactNode, color?: string }) => (
  <span className={`px-2.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border transition-all ${color === 'emerald' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
      ...
      <div className="relative aspect-square overflow-hidden rounded-[24px] bg-zinc-50">
        ...
        {product.status === 'Flagged' && (
          <div className="absolute inset-0 bg-red-600/20 backdrop-blur-[4px] flex items-center justify-center p-6 text-center">
            <span className="bg-red-600 text-white text-[9px] font-black uppercase px-4 py-2 rounded-full shadow-2xl">Under Review</span>
          </div>
        )}
      </div>
      <div className="pt-6 flex-1 flex flex-col">
        <div className="flex items-center justify-between mb-3">
          ...
          {vendor?.isVerified && (
            <div className="flex items-center text-indigo-500 group/v relative">
              ...
              <span className="absolute bottom-full right-0 mb-2 opacity-0 group-hover/v:opacity-100 transition-opacity bg-zinc-950 text-white text-[8px] px-2 py-1 rounded-md pointer-events-none whitespace-nowrap uppercase font-black">Verified Seller</span>
            </div>
          )}
        </div>
        ...
        <div className="mt-auto flex justify-between items-center pt-5 border-t border-zinc-50">
          <div className="flex flex-col">
            <span className="text-[9px] font-black text-zinc-300 uppercase tracking-widest">Price</span>
      ...
      <div className="flex flex-col lg:flex-row gap-16">
        ...
        <aside className="w-full lg:w-80 space-y-12">
          <div>
            <nav className="space-y-2">
              <p className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.4em] mb-4">Categories</p>
              {['All', ...CATEGORIES].map(cat => (
                <button
                  key={cat}
                  onClick={() => props.setActiveCategory(cat)}
                  className={`w-full text-left px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${props.activeCategory === cat ? 'bg-zinc-950 text-white shadow-xl' : 'text-zinc-400 hover:text-zinc-950 hover:bg-zinc-50'
                    }`}
                >
                  {cat}
                </button>
        ...
        <div className="flex-1">
          <div className="mb-12 flex justify-between items-end">
            <div>
              <h1 className="text-3xl md:text-5xl font-black text-zinc-950 tracking-tighter mb-4">Market Terminal</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-8">
            ...
            {props.products.length === 0 && (
              <div className="col-span-full py-40 text-center flex flex-col items-center">
                ...
                <h3 className="text-xl font-black text-zinc-950 uppercase tracking-widest">Empty Store</h3>
                <p className="text-zinc-500 font-bold uppercase tracking-widest text-[9px] mb-8">No inventory matches your search</p>
                <button onClick={() => props.setActiveCategory('All')} className="mt-8 text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:text-indigo-800 transition-colors">Clear Industry Filters</button>
              </div>
            )}
          </div>
        </div>
