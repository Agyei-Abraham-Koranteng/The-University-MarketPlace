...
const Badge = ({ children, status }: { children: React.ReactNode, status?: string }) => (
  <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all ${status === 'Completed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8">
        <div>
          ...
          <p className="text-zinc-400 text-xs font-bold mt-2 uppercase tracking-widest">Digital Wallet: {vendor.storeName}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
        <div className="bg-white p-6 rounded-[24px] border border-zinc-100 shadow-sm relative overflow-hidden group">
          <div className="relative z-10">
            <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-2">Available Funds</p>
        ...
        <div className="bg-white p-6 rounded-[24px] border border-zinc-100 shadow-sm relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <p className="text-[10px] font-bold text-amber-500 mt-4 flex items-center">
              <i className="fas fa-clock mr-2"></i> In Verification
            </p>
          </div>
        ...
        <div className="bg-zinc-950 p-6 rounded-[24px] shadow-2xl relative overflow-hidden group">
          <div className="relative z-10">
            <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest mb-2">Total Withdrawn</p>
            ...
            <p className="text-[10px] font-bold text-zinc-400 mt-4 flex items-center">
              <i className="fas fa-arrow-up-right-from-square mr-2"></i> Earnings History
            </p>
          </div>
      ...
      {/* Transaction History */}
      <div className="bg-white rounded-[40px] border border-zinc-100 shadow-sm overflow-hidden p-3">
        <div className="p-8 border-b border-zinc-50 flex justify-between items-center">
          <h3 className="text-lg font-black text-zinc-950 tracking-tight">Transaction History</h3>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-zinc-50">
              <th className="px-10 py-6 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Transaction ID</th>
              <th className="px-10 py-6 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Transaction Type</th>
              <th className="px-10 py-6 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Amount</th>
              ...
              <tr key={t.id} className="hover:bg-zinc-50/50 transition-all group">
                ...
                <td className="px-10 py-8">
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs ${t.type === 'Sale' ? 'bg-indigo-50 text-indigo-600' : 'bg-zinc-100 text-zinc-500'
                      }`}>
      ...
      {isWithdrawing && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[32px] shadow-2xl p-10 animate-in zoom-in-95 duration-300">
            <h3 className="text-3xl font-black text-zinc-950 tracking-tighter mb-8 text-center">Action: Withdrawal</h3>

            <form onSubmit={handleWithdrawal} className="space-y-8">
              <div>
                <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-4 block">Withdrawal Amount ($)</label>
              ...
              <div>
                <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-4 block">Payment Method</label>
                <select
                  value={method}
              ...
              <div className="bg-amber-50 p-6 rounded-3xl border border-amber-100">
                <p className="text-[9px] font-black text-amber-600 uppercase tracking-widest flex items-center">
                  <i className="fas fa-triangle-exclamation mr-2"></i> Safety Verification
                </p>
