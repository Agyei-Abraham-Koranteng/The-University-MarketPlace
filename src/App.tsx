...
import {
  UserRole, Product, Vendor, Profile,
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
          ...
          <div className="relative w-full max-w-2xl glass rounded-3xl shadow-4xl overflow-hidden animate-in zoom-in-95 slide-in-from-top-4 duration-300">
            ...
            <div className="p-4 max-h-96 overflow-y-auto">
              ...
              {filteredProducts.slice(0, 5).map(p => (
                <button
                  key={p.id}
                  onClick={() => { setSelectedProductId(p.id); navigate(`/product/${p.id}`); setShowSearchOverlay(false); }}
      ...
      <main>
        <Routes>
          <Route path="/" element={<HomeView onEnterMarket={() => navigate('/marketplace')} onStartSelling={() => requireAuth('/vendor/onboarding')} />} />
          ...
          <Route path="/marketplace" element={
            <MarketplaceView
              ...
              onSelectProduct={(id) => { setSelectedProductId(id); navigate(`/product/${id}`); }}
            />
          } />
