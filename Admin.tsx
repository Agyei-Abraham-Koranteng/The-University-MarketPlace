    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-lg rounded-[48px] shadow-4xl overflow-hidden p-10 md:p-12 animate-in zoom-in-95 duration-300">
         ...
         <div className="flex gap-4 mt-12">
            ...
            <button onClick={() => { props.onUpdateUser(editingUser); setEditingUser(null); }} className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-100">Commit Changes</button>
         </div>
      </div>
    </div>
  );
  ...
  const EditProductModal = () => editingProduct && (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-lg rounded-[48px] shadow-4xl overflow-hidden p-10 md:p-12 animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
         ...
         <div className="flex gap-4 mt-12">
            <button onClick={() => { props.onDeleteProduct(editingProduct.id); setEditingProduct(null); }} className="flex-1 bg-red-50 text-red-600 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest">Delete Asset</button>
            <button onClick={() => { props.onUpdateProduct(editingProduct); setEditingProduct(null); }} className="flex-1 bg-zinc-950 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest">Sync Database</button>
         </div>
      </div>
    </div>
  );

  const ApplicationReviewModal = () => selectedApp && (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-4xl rounded-[48px] shadow-4xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col lg:flex-row h-[80vh]">
        ...
        <div className="w-full lg:w-[380px] p-12 flex flex-col justify-between">
           ...
           <div className="space-y-4">
              <button 
                onClick={() => { props.onRejectApplication(selectedApp.id); setSelectedApp(null); }}
              ...
              <button 
                onClick={() => { props.onApproveApplication(selectedApp.id); setSelectedApp(null); }}
